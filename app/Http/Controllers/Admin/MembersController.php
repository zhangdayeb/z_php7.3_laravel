...
class MembersController extends AdminBaseController
{
    ...
    **/
        ...
        try{
            DB::transaction(function() use ($old_top_id,$top_id,$member,$top){
                ...
                if(!$old_top_id){
                    ...
                    // 修改代理的 agent_fd_rate 中 type = 3 的点位
    ...
    public function money_report(Request $request){
        ...
        // 统计
		$total_debit = $all_data->sum('moneylogSumDebit'); //管理员总扣款
    ...
    // 刷新会员接口的余额
    public function refresh_api(MemberApi $member_api){
        ...
        $res = json_decode(app(SelfService::class)->balance($params['api_code'],$params['name']),true);

        if($res['Code'] != 0){
            return $this->failed(trans('res.member.msg.balance_error').$res['Message']);
        }

        $money = sprintf("%.2f",$res['Data']['balance']);
