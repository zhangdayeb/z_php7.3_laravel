...
if(env('ADMIN_URL')){
    Route::domain(env('ADMIN_URL'))->group(function($router){
        $router->get("/","Admin\LoginController@showLoginForm");
    });
}

if(env('AGENT_URL')){
    Route::domain(env('AGENT_URL'))->group(function($router){
        $router->get("/","Agent\LoginController@showLoginForm");
    });
}
...
Route::domain(env('APP_URL'))->group(function(){
    ...
    Route::namespace("Web")->name("web.")->middleware('maintenance')->group(function(){
        ...
        /**
        Route::get("activity/{activity}","IndexController@activity_detail")->name('activity.detail');

        Route::post('activity/apply/config','IndexController@activity_apply_config');
        Route::post('activity/apply/result','IndexController@activity_apply_result');
        Route::post('activity/apply/form/{activity}','IndexController@activity_apply_form');
        Route::post('activity/apply/{activity}','IndexController@activity_apply');

        Route::get('wheel','IndexController@wheel');
        Route::get('wheel/scrollMessages','IndexController@scroll_messages');
        Route::get('wheel/myDetail/{name}','IndexController@wheel_detail');
        Route::get('wheel/iWantRaffle/{name}','IndexController@wheel_award');

        Route::get('levelup/slot','IndexController@slotLevelUp')->name('levelup.slot');
        Route::post('levelup/slot/search','IndexController@slotLevelSearch')->name('levelup.slot.search');

        Route::any('levelup/live','IndexController@liveLevelUp')->name('levelup.live');

        Route::get('credit_pay/index','IndexController@creditPay')->name('credit_pay.index');
        Route::get('credit_pay/record','IndexController@creditRecord')->name('credit_pay.record');
        Route::post('credit_pay/list','IndexController@creditRecordList')->name('credit_pay.list');
        Route::get('credit_pay/borrow','IndexController@creditBorrow')->name('credit_pay.borrow');
        Route::post('credit_pay/borrow','IndexController@creditBorrowPost')->name('credit_pay.borrow.post');
        Route::get('credit_pay/lend','IndexController@creditLend')->name('credit_pay.lend');
        Route::post('credit_pay/lend','IndexController@creditLendPost')->name('credit_pay.lend.post');
        Route::post('credit_pay/search','IndexController@creditSearchPost')->name('credit_pay.search');
        Route::post('credit_pay/check','IndexController@creditCheckMember')->name('credit_pay.check');
         */
    });
...
Route::namespace("Agent")->domain(env('AGENT_URL') ?? env('APP_URL'))->name("agent.")->prefix("agent")->middleware(['maintenance','language'])->group(function(){
...
Route::namespace("Admin")->domain(env('ADMIN_URL') ?? env('APP_URL'))->name("admin.")->prefix("admin")->middleware("language")->group(function () {
    ...
    Route::middleware("auth:web")->group(function(){
        ...
        Route::middleware(["rbac.admin"])->group(function () {
            ...
            // Route::post("messgaes/mark/{message}","MessagesController@post_mark_deal")->name('messages.post_mark_deal');
